...
- **nash**: 零和ナッシュ均衡ソルバー / 選出最適化（`pkdx select`） / メタ乖離分析（`pkdx meta-divergence`）。`pkdx select` は回復技（じこさいせい等）・ドレイン技（ギガドレイン等）・やどりぎのタネ・ねむる・いたみわけ・ねがいごと・天候依存回復（つきのひかり等）を考慮する。天候設定技を撃った後に天候依存回復が強化される動的挙動や、ヘドロえき / ふみん / マジックガード等の関連特性による回復技・状態異常の干渉もモデル化される。さらに いかく（登場時に相手の攻撃ランクを下げ、ばんけん・ミラーアーマー・びびり・あまのじゃく・無効化特性との相互作用を含む）、あまのじゃく（技による能力変化の反転）、ばけのかわ（初撃無効化と最大HP/8 の剥がれダメージ）も選出評価に反映される。
...
3. 利用可能なポケモン、道具、技のプールを常にDBへ問い合わせ確認する。ユーザーが指定したフォーマット外の情報をフィードバックしてはならない
    例: `Champions M-C` レギュレーション選択中に、準伝説ポケモンやノココッチを案内してしまう（M-Cフォーマット外）
    例: `Champions M-C` レギュレーション選択中に、こだわりハチマキやとつげきチョッキを案内してしまう（M-Cフォーマット外）
